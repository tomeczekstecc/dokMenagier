...
router.post('/register', async (req, res) => {
  ...
  try {
    ...
    await user.save()
    return res.json({
      msg: 'Utworzono użytkownika', user_id: user.id
    })
  } catch (error) {
    console.log(error);
  }
...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    if (!user) {
      return res.json({ msg: 'Niepoprawne dane logowania - brak użytkownika lub błędne hasło.' })
    }
    //2. Compare passwords
    ...
    if (!isMatch) {
      return res.json({
        msg: 'Niepoprawne dane logowania - brak użytkownika lub błędne hasło.'
      })
    } else {
      //3. Create tokens
    ...
    res.json({ error: `Błąd: ${error.message}` });
  }
